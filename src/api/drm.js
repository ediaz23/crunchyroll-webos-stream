...
export const getStreams = async (profile, params) => {
    ...
    try {
        ...
        out = await api.drm.getStream({ account, fnConfig: { cache: false }, ...params })
    } catch (error) {
...
export const deleteToken = async (profile, params) => {
    try {
        const account = await getContentParam(profile)
        await api.drm.deleteToken({ account, fnConfig: { cache: false }, ...params })
    } catch (error) {
        await translateError(error)
    }
}
...
export const keepAlive = async (profile, params) => {
    ...
    try {
        const account = await getContentParam(profile)
        out = await api.drm.keepAlive({ account, fnConfig: { cache: false }, ...params })
    } catch (error) {
        await translateError(error)
    }
    return out
